...
if 'v75-naedeok2-apartment-only' not in text:
    raise SystemExit('v75 source logic missing from index')
data=Path('/tmp/v75-precomputed.json').read_text(encoding='utf-8')
obj=json.loads(data)
if obj.get('version')!='v75-naedeok2-apartment-only':
    raise SystemExit('wrong export version')
