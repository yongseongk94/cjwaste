...
async function matchVillageFacility(target){
  ...
  for(const district of target.districts){
    for(const kind of ['마을회관','경로당']){
      const query='청주시 청원구 '+district+' '+name+' '+kind;
      let rows=await keyword(query);
      if(!rows.length)rows=await keyword('청주시 청원구 '+district+' '+stem+' '+kind);
      const matches=rows.filter(r=>{
        const n=compact(r.place_name),a=String(r.address_name||'');
        if(!a.includes('청원구')||!a.includes(district))return false;
        if(kind==='마을회관'?!/마을회관/.test(n):!/경로당|노인정/.test(n))return false;
        const stripped=n.replace(/(?:마을회관|경로당|노인정).*$/,'').replace(/마을$/,'');
        if(!(stripped===name||stripped===stem))return false;
  ...
  return {...target,status:results.length===1?'matched':results.length?'ambiguous':'unresolved',facilities:results};
}
